...
def run_exercise(ex_name):
    ...
    Config.variant          = "full"
    Config.SAVE_DIR         = f"checkpoints/{ex_name}/full"

    # 固定最优超参
    Config.use_augment            = False
    Config.lambda_rank            = 0.05
    Config.use_temporal_attention = False

    os.makedirs(Config.SAVE_DIR, exist_ok=True)

    seeds = getattr(Config, "seeds", [2024, 2025, 2026])
    ...
    print(f"[{ex_name}] SRCC={result['srcc_mean']:.4f}±{result['srcc_std']:.4f} "
...
def main():
    ...
    # 保存汇总表
    out_path = "results/all_exercises_summary.csv"
